...
func initialize(game_data):
	...
	for food in game_data['foods']:
		...
		var new_pos = Vector2(randi_range(offset,margin.size.x-offset),randi_range(offset,margin.size.y/2))
		new_food.placed.connect(foodPlaced)
		new_food.initialize(new_pos, food, scale_factor, food_count)
		food_count += 1
